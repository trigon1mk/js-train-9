...
function findIndexByProperty(arr, name) {
  ...
  const find = arr.findIndex((element) => element.name === name);
  return find;
...
function findLastEvenNumber(arr) {
  ...
  const lastNum = arr.findLast((element) => element % 2 === 0);
  if (lastNum) {
    return lastNum;
  } else {
...
function getPositiveArrayValues(arr) {
  ...
  const fromArray = Array.from(positiveArrayValues);
  const plusArr = fromArray.filter((num) => num > 0);
  return plusArr;
...
function removeAndDouble(arr, startIdx) {
  ...
  const remArr = arr.splice(startIdx);
...
function sumAndPrint(arr) {
  ...
  const initValue = 0;
  const sum = arr.reduce((acum, curValur) => acum + curValur, initValue);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const filterArr = mapArr.filter((num) => num > threshold);
  return filterArr;
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = sliceArr.reduce((acum, curValur) => acum + curValur, initValue);
...
function insertElementAt(arr, index, element) {
  ...
  const elemSp = arr.splice(index, 0, element);
  return arr;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  ...
  const flatMapArr = arr.flatMap((num) => Array(n).fill(num));
  return flatMapArr;
...
function findLongestWord(arr) {
  ...
  const maxLength = arr.reduce((acum, curValur) => {
    return curValur.length > acum.length ? curValur : acum;
  }, initValue);
...
function findDuplicateElements(arr) {
  ...
  const filtArr = arr.filter((el, index) => {
    return arr.indexOf(el) !== index;
  });

  return [...new Set(filtArr)];
